...
# write to a csv file
subject_count.to_csv('subject_count.csv', index=False)
...
# write to a csv file
leave_to_count.to_csv('leave_to_count.csv', index=False)
...
plt.xlabel('Leave Year')
plt.ylabel('Unique Author Count')
plt.title('Unique Author Count by Leave To and Leave Year')
plt.legend()
plt.grid(True)
plt.savefig('leave_to_count.png')
plt.show()
